...
function loadData(data) {
    ...
    for (var i = 0; i < data.length; i++){
        ...
        if (validateCity(data[i].split(';')[5]) == false && data[i].split(';')[14] != '0'){
            var city = new Object();
    ...
    var todo = arraySectors[0].name + " - "+ TotalSector+" Registro(s)";
...
function countSector(nameCity) {
    for (var i = 0; i < arrayBusinesses.length; i++){
        if (arrayBusinesses[i][5] == nameCity && arrayBusinesses[i][14] != '0'){
            if (validateSector(arrayBusinesses[i][10]) == false){
                ...
                for (var j = 0; j < arraySectors.length; j++){
                    if (arraySectors[j].name == arrayBusinesses[i][10]){
                        arraySectors[j].quantity = arraySectors[j].quantity + parseInt(arrayBusinesses[i][14], 10);
    ...
    for(var i = 0; i < arraySectors.length; i++){
	    total=total+arraySectors[i].quantity;
    }
}

function countSubSector(nameCity, sector) {
    for (var i = 0; i < arrayBusinesses.length; i++){
        if (arrayBusinesses[i][5] == nameCity && arrayBusinesses[i][10] == sector && arrayBusinesses[i][14] != '0'){
            if (validateSubSector(arrayBusinesses[i][9]) == false){
                ...
                for (var j = 0; j < arraySubSectors.length; j++){
                    if (arraySubSectors[j].name == arrayBusinesses[i][9]){
                        arraySubSectors[j].quantity = arraySubSectors[j].quantity + parseInt(arrayBusinesses[i][14], 10);
                    }
                }
            }
        }
...
function selectSector(arraySectors){
	...
	var todo;
    todo=arraySectors[arraySectors.selectedIndex].value+" - "+ TotalSector+" Registro(s)";
	document.getElementById('TotalSector').innerHTML = todo;
